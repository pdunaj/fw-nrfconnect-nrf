...
static void format_address(char *buffer, size_t buffer_size, const uint8_t *addr_m8, size_t addr_size)
{
	if (addr_size == 0) {
		buffer[0] = 0;
		return;
...
static bool check_neighbors(otInstance *instance)
{
	...
	while (otThreadGetNextNeighborInfo(instance, &iterator, &info) == OT_ERROR_NONE) {
		...
		LOG_INF("Neighbor addr:%s age:%u", log_strdup(addr_str), (unsigned)info.mAge);
...
static void on_thread_state_changed(uint32_t flags, void *context)
{
	...
	struct openthread_context *ot_context = static_cast<openthread_context *>(context);
