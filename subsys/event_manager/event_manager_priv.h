...
#define _EVENT_SUBSCRIBERS_DECLARE(ename)							\
	extern const struct event_subscriber _EVENT_SUBSCRIBERS_START(ename, )[];		\
	extern const struct event_subscriber _EVENT_SUBSCRIBERS_STOP(ename, )[];		\
...
#define _EVENT_SUBSCRIBERS_DEFINE(ename)	\
	_EVENT_SUBSCRIBERS_EMPTY(ename, _a )	\
	_EVENT_SUBSCRIBERS_EMPTY(ename, _z )
...
#define _EVENT_TYPE_DEFINE(ename, init_log_en, log_fn, trace_data_pointer)			\
	...
	__attribute__((__section__("event_types"))) = {						\
		...
		.subs_start			= (const struct event_subscriber*)&(_CONCAT(_EVENT_SUBSCRIBERS_SECTION_PREFIX(ename, _a), empty)),	\
		.subs_stop			= (const struct event_subscriber*)&(_CONCAT(_EVENT_SUBSCRIBERS_SECTION_PREFIX(ename, _z), empty)),	\
...
#if 0
		.subs_start			= _EVENT_SUBSCRIBERS_START(ename, ),		\
		.subs_stop			= _EVENT_SUBSCRIBERS_STOP(ename, ),
#endif
